...
static int led_dev_init(void)
{
    ...
    led_dev_o.dev_va.CCM_CCGR1_VA = ioremap(CCM_CCGR1_PHY, 4);
    led_dev_o.dev_va.IOMUXC_SW_MUX_CTL_PAD_GPIO1_IO04_VA = ioremap(IOMUXC_SW_MUX_CTL_PAD_GPIO1_IO04_PHY, 4);
    led_dev_o.dev_va.IOMUXC_SW_PAD_CTL_PAD_GPIO1_IO04_VA = ioremap(IOMUXC_SW_PAD_CTL_PAD_GPIO1_IO04_PHY, 4);
    led_dev_o.dev_va.GPIO1_DR_VA = ioremap(GPIO1_DR_PHY, 4);
    led_dev_o.dev_va.GPIO1_GDIR_VA = ioremap(GPIO1_GDIR_PHY, 4);

    // init led on GPIO1.IO04 manually begin
    // CCM
    val = readl(led_dev_o.dev_va.CCM_CCGR1_VA);
    // reset original value
    val &= ~(3 << 26);
    // write desired value
    val |= (3 << 26);
    writel(val, led_dev_o.dev_va.CCM_CCGR1_VA);

    // MUX
    writel(5, led_dev_o.dev_va.IOMUXC_SW_MUX_CTL_PAD_GPIO1_IO04_VA);
    // PAD
    writel(0x10B0, led_dev_o.dev_va.IOMUXC_SW_PAD_CTL_PAD_GPIO1_IO04_VA);

    // GDIR (set input or output, set to 1 as output)
    val = readl(led_dev_o.dev_va.GPIO1_GDIR_VA);
    val &= ~(1 << 4);
    val |= (1 << 4);
    writel(val, led_dev_o.dev_va.GPIO1_GDIR_VA);
    
    // DR (data register)
    val = readl(led_dev_o.dev_va.GPIO1_DR_VA);
    val |= (1 << 4);
    writel(val, led_dev_o.dev_va.GPIO1_DR_VA);

    // init led on GPIO1.IO04 manually end

    // assign dev_t
    alloc_chrdev_region(&(led_dev_o.dev_id), 0, 1, LED_DEV_NAME);

    // cdev
    cdev_init(&(led_dev_o.dev_cdev), &led_dev_fops);
    cdev_add(&(led_dev_o.dev_cdev), led_dev_o.dev_id, 1);
    
    // class & device
    led_dev_o.dev_class = class_create(THIS_MODULE, LED_DEV_NAME);
    led_dev_o.dev_device = device_create(led_dev_o.dev_class, NULL, led_dev_o.dev_id, NULL, LED_DEV_NAME);
...
static void led_dev_exit(void)
{
    ...
    // cdev
    cdev_del(&(led_dev_o.dev_cdev));

    // device & class
    device_destroy(led_dev_o.dev_class, led_dev_o.dev_id);
    class_destroy(led_dev_o.dev_class);
