...
int platform_driver_led_probe(struct platform_device *pdev)
{
    ...
    // 获取reg里面的信息并映射成va
    ccm_ccgr1_va = of_iomap(led_node, 0);
    iomux_sw_mux_ctl_pad_va = of_iomap(led_node, 1);
    iomux_sw_pad_ctl_pad_va = of_iomap(led_node, 2);
    gpio_gdir_va = of_iomap(led_node, 3);
    gpio_dr_va = of_iomap(led_node, 4);
